...
// We accept categories as prop to show in dropdown
export function ActionFab({ categories }: { categories: any[] }) {
    const [isOpen, setIsOpen] = useState(false)
    const [modalMode, setModalMode] = useState<'task' | 'category' | null>(null)
            ...
            <div className="fixed bottom-8 right-8 flex flex-col items-end gap-4 z-50">
                {isOpen && (
                    <div className="flex flex-col items-end gap-3 animate-in slide-in-from-bottom-5 fade-in duration-200">
                        <button
                            onClick={() => { setModalMode('category'); setIsOpen(false) }}
...
export function CreateTaskModal({ categories, onClose, initialCategoryId }: { categories: any[], onClose: () => void, initialCategoryId?: string }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-sm shadow-2xl p-6 relative">
                ...
                <h2 className="text-xl font-bold mb-4 text-zinc-900 dark:text-white">New Category</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Name</label>
                        <input name="name" required className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2" placeholder="e.g. Work" />
                    </div>
                    <div>
                    ...
                    <div className="pt-2 flex justify-end gap-2">
                        <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-zinc-600 hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-800 rounded-lg">Cancel</button>
                        <button type="submit" disabled={loading} className="px-4 py-2 text-sm font-medium bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg">Create Category</button>
                    </div>
                </form>
            </div>
        </div>
